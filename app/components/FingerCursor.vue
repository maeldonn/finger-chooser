...
<template>
  ...
  >
    <div
      ...
      :style="{ borderColor: props.finger.color, boxShadow: `${glowIntensity} ${props.finger.color}`, color: props.finger.color }"
    />
    <div
      class="w-12 h-12 rounded-full bg-white/20 backdrop-blur-sm finger-core"
      :class="{ winner: props.isWinner }"
      :style="{ backgroundColor: props.isWinner ? props.finger.color : 'transparent' }"
    />
  </div>
</template>
