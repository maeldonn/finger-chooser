...
- **Installable PWA** - powered by `@vite-pwa/nuxt` with auto-updating service worker, manifest, and reusable icons (`public/logo.png`, `pwa-192x192.png`, `pwa-512x512.png`, `favicon.ico`).
...
- All icons come from `public/logo.png`. If you replace that source image, regenerate `pwa-192x192.png`, `pwa-512x512.png`, and `favicon.ico` (ImageMagick's `magick convert` works well).
